...
Maestro, dispositivo físico, armazenamento local seguro, emissão/revogação de sessões, passkeys, PIN, recovery, Terraform e infraestrutura de nuvem estão apenas planejados. A fronteira HTTP de autenticação por sessão já existe como opção técnica para o modo financeiro, mas o padrão local continua desabilitado e não há ambiente ou fornecedor de produção.
