...
- consulta iOS de metas financeiras e valores protegidos pela FinancialAPI existente, com estados explícitos, acessibilidade e sem movimentação financeira;
...
Maestro, dispositivo físico, armazenamento local seguro, passkeys, PIN, recovery, Terraform e infraestrutura de nuvem estão apenas planejados. A fronteira HTTP de autenticação por sessão e seu fluxo opt-in de emissão/revogação local já existem como opção técnica para o modo financeiro, mas o padrão local continua desabilitado e não há ambiente ou fornecedor de produção.
