...
- **Campos modelados:** `Expense` e `Income` são agregados separados e representam ID, UserID, tipo, descrição, valor, data/hora, timezone financeiro, origem e `CategoryID` opcional; somente Expense possui forma de pagamento. UserID é um identificador opaco do modelo e não é considerado intrinsecamente anônimo ou sintético. O catálogo global de sistema isolado não identifica um usuário; Category é classificação, não salário, empregador, source ou conta, mas sua associação a uma transaction pode revelar contexto sensível.
...
- **Cliente iOS:** o app mantém rascunho Expense/Income, seleção/catálogo de Category, preview e chave idempotente somente em memória, sem UserDefaults, banco local, analytics, telemetria ou owner. Descrição e classificação podem conter ou revelar informação financeira sensível, por isso minimização e proibição de dados reais em fixtures continuam obrigatórias.
