...
- Emissão/revogação de sessões, multiusuário, recovery e rate limiting distribuído permanecem planejados; não usar o contexto single-owner temporário como autorização para beta externo. A fronteira opt-in atual ainda não é um release gate de produção.
- Qualquer beta externo exige o [LGPD Readiness Gate](../privacy/privacy-by-design-checklist.md).
- Não usar a UI atual com dados reais: não há armazenamento local seguro nem recovery de operação após restart, e a autenticação opt-in ainda não cobre emissão, revogação, multiusuário ou recovery. Descrições de Income podem revelar renda ou empregador; Category também pode revelar contexto sensível, mesmo sendo uma classificação estruturada.
