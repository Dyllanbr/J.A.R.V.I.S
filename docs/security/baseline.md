...
- Não usar a UI atual com dados reais: não há autenticação, autorização, armazenamento local seguro nem recovery de operação após restart. Descrições de Income podem revelar renda ou empregador mesmo sem campos estruturados específicos.
