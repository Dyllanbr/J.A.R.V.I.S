...
- Não há credenciais reais, banco de produção ou coleta operacional de usuários reais. A persistência atual contém somente dados sintéticos local/CI; a autenticação HTTP por sessão existe apenas como capacidade opt-in para o modo financeiro.
...
- Leituras e replays permanecem owner-scoped. FKs compostas e constraints cross-type protegem ownership e impedem associar operação ou audit event ao tipo financeiro errado. A fronteira HTTP opt-in valida bearer opaco em sessão read-only, rejeita credencial ausente/inválida com 401 e subject divergente com 403; emissão usa um segredo de bootstrap configurado no servidor e revogação só afeta a sessão atual. Multiusuário e recovery ainda não existem.
- O cliente iOS usa URLSession efêmera, desabilita cache/cookies persistentes e não registra payloads. Opcionalmente, `JARVIS_IOS_API_BEARER` fornece um bearer configurado na execução para o modo HTTP autenticado; ele não é emitido nem persistido pelo app e o cliente nunca contém owner. Somente o build `DEBUG` aceita o stub sintético de UI test.
...
- O Simulator em loopback é o único alvo integrado atual. O backend não foi aberto à LAN para dispositivo físico; a autenticação opt-in atual é uma fronteira de servidor e não substitui o hardening necessário para exposição externa.
...
- Multiusuário, recovery e rate limiting distribuído permanecem planejados; não usar o contexto single-owner temporário como autorização para beta externo. A fronteira opt-in, incluindo bootstrap local, ainda não é um release gate de produção.
- Qualquer beta externo exige o [LGPD Readiness Gate](../privacy/privacy-by-design-checklist.md).
- Não usar a UI atual com dados reais: não há armazenamento local seguro nem recovery de operação após restart, e a autenticação opt-in ainda não cobre multiusuário ou recovery. Descrições de Income podem revelar renda ou empregador; Category também pode revelar contexto sensível, mesmo sendo uma classificação estruturada.
