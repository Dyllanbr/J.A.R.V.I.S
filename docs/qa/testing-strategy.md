...
- Testes de integração contra PostgreSQL 18.6 real para migrations 001–008, seed/FK composta de Category, persistência dedicada de Recurrence e CreditCard, CardPurchase/InstallmentPlan, suppression de suggestions, matriz de tipos/payment method/audit/operação, ownership, limites Unicode/whitespace, corrupção semântica, rollback, concorrência, restart, `DateStyle` independente, replay histórico e DOWN seguro sob lock/guard.
...
- Gate local fail-closed para os fluxos reais `Simulator → SwiftUI → URLSession → Go → PostgreSQL`, com fixtures sintéticas únicas e pós-condições diretas no banco. Expense/Income preservam `1 transaction + 1 AuditEvent + 1 idempotency record` por tipo e `category_id` esperado; Income também exige `payment_method IS NULL`. Recurrence, CreditCard e InstallmentPlan possuem persistência, audit events e idempotência próprias, e não produzem writes indevidos nas tabelas legadas. CardPurchase à vista cria somente a Expense total; CardPurchase parcelado cria a Expense total e um plano, sem Expenses futuras. O fluxo real de suggestion prova três Expenses de evidência, zero Recurrence antes do `Confirm`, exatamente uma Recurrence após a confirmação e zero suppression nesse caminho.
