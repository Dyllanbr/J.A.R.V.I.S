...
- Testes de integração contra PostgreSQL 18.6 real para migrations 001–009, seed/FK composta de Category, persistência dedicada de Recurrence e CreditCard, CardPurchase/InstallmentPlan, MonthlyBudget, SafeAvailable, PurchaseSimulation, suppression de suggestions, matriz de tipos/payment method/audit/operação, ownership, limites Unicode/whitespace, corrupção semântica, rollback, concorrência, restart, `DateStyle` independente, replay histórico e DOWN seguro sob lock/guard.
- Testes de reserva/replay/conflito idempotente para `CREATE_EXPENSE`, `CREATE_INCOME`, `CREATE_RECURRENCE`, `CANCEL_RECURRENCE`, operações de CreditCard e `CREATE_CARD_PURCHASE`, incluindo payload igual/diferente concorrente, retry após falhas, escopo por owner, defesa cross-type e replay histórico da criação mesmo após cancelamento posterior.
...
- XCTest para Money input, codecs, data civil, modelos discriminados, Recurrence e RecurrenceSuggestion, cliente HTTP, cancelamento, catálogo compartilhado, PurchaseSimulation, máquinas de estados/retry, stale Preview, histórico misto e reconciliação concorrente de Recurrences/suggestions. As regressões de concorrência usam `CheckedContinuation`/barreiras determinísticas, sem sleeps ou polling. XCUITest com stub `DEBUG` explícito cobre Expense/Income categorizados, picker/filtros de Category, alternância repetida das três tabs por identifiers semânticos, History normal/erro/misto, Recurrences create/list/cancel, suggestions com Review/Confirm ou “Agora não”, simulação de compra, acessibilidade e Dynamic Type extremo.
- Gate local fail-closed para os fluxos reais `Simulator → SwiftUI → URLSession → Go → PostgreSQL`, com fixtures sintéticas únicas e pós-condições diretas no banco. Expense/Income preservam `1 transaction + 1 AuditEvent + 1 idempotency record` por tipo e `category_id` esperado; Income também exige `payment_method IS NULL`. Recurrence, CreditCard e InstallmentPlan possuem persistência, audit events e idempotência próprias; MonthlyBudget possui persistência owner/mês com substituição atômica, sem audit event ou idempotência financeira adicional, e nenhuma dessas leituras produz writes indevidos nas tabelas legadas. CardPurchase à vista cria somente a Expense total; CardPurchase parcelado cria a Expense total e um plano, sem Expenses futuras. SafeAvailable é validado por período explícito, breakdown, marcador BUDGET e isolamento entre owners, com leitura repetida sem novas rows. O fluxo real de suggestion prova três Expenses de evidência, zero Recurrence antes do `Confirm`, exatamente uma Recurrence após a confirmação e zero suppression nesse caminho.
...
As subcapacidades 4A — CreditCard e 4B — CardPurchase + InstallmentPlan estão **Verified**. A auditoria independente final da Stage 4 foi aprovada com P0=0, P1=0, P2=0 e P3=0; o Incremento 4B foi mergeado pelo PR #75 no commit `526cc855`. As evidências cobrem domínio/aplicação, migrations 007/008, PostgreSQL real, HTTP/OpenAPI, Playwright, XCTest/XCUITest, acessibilidade, replay/idempotência, cancellation preview/cancelamento e E2E real Simulator → SwiftUI → URLSession → Go → PostgreSQL. O restante do Incremento 4, incluindo Statement/faturas completas e compromissos futuros além de InstallmentPlan, permanece planejado.

O Incremento 5 — Orçamento mensal e Disponível Seguro está **Verified**. SafeAvailable e MonthlyBudget foram validados em Domain/Application, PostgreSQL (migration 009), HTTP/OpenAPI, iOS e E2E real Simulator → SwiftUI → URLSession → Go → PostgreSQL. A evidência inclui fórmula determinística, valores zero/negativos, breakdown, marcador BUDGET, owner isolation A/B, leituras sem writes e pós-condições SQL. O CI do PR #83 concluiu com sucesso e o merge commit é `b7e385f`.

O simulador “Posso comprar?” do Incremento 6 está **VERIFICADO** dentro do escopo entregue. A evidência cobre Domain/Application, PostgreSQL read-only, POST `/v1/purchase-simulations`, OpenAPI 0.13.0, iOS na área de Cartões, compra à vista/parcelada, impacto e premissas, teste real e regressões dos fluxos de cartões. O PR #86 foi mergeado com sucesso na `main` no commit `5691351c5cb462d1abf6e1d50e68354ae7be522`; os gates Go e iOS do CI passaram. As declarações backend de metas e valores protegidos foram implementadas nas Stages 1–3 do Incremento 6 com testes Go e integração PostgreSQL; a Stage 4 adicionou consulta iOS e testes focados de modelos, API, ViewModel e UI. E2E iOS dedicado, progresso e alertas permanecem planejados. O job iOS do PR #90 foi cancelado no limite de 26m pelo runner, portanto a Stage 3 não é classificada como Verified independente.
