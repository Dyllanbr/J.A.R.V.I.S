...
func TestMigration006FreshSchemaConstraintsSafeDownAndReapply(t *testing.T) {
	...
	withConnection(t, ctx, pool, func(connection *pgx.Conn) {
		...
		if err := migrations.Down(ctx, connection); err != nil {
			t.Fatalf("migration 011 DOWN after migration 006 reapply failed: %v", err)
		}
		if err := migrations.Down(ctx, connection); err != nil {
			t.Fatalf("migration 010 DOWN after migration 006 reapply failed: %v", err)
		}
		if err := migrations.Down(ctx, connection); err != nil {
...
func TestMigration006UpgradeFrom005PreservesExistingFinancialAndRecurrenceData(t *testing.T) {
	...
	withConnection(t, ctx, pool, func(connection *pgx.Conn) {
		...
		if err := migrations.Down(ctx, connection); err != nil {
			t.Fatalf("migration 011 DOWN after migration 006 upgrade failed: %v", err)
		}
		if err := migrations.Down(ctx, connection); err != nil {
			t.Fatalf("migration 010 DOWN after migration 006 upgrade failed: %v", err)
		}
		if err := migrations.Down(ctx, connection); err != nil {
