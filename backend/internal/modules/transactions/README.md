...
Ambos também aceitam `CategoryID` técnico e opcional. Ausência significa “Sem categoria”; `expense.other`/`income.other` são classificações reais e não defaults. O value object valida somente a forma técnica estável, enquanto existência, display name, ordenação e applicability Expense/Income vêm de `CategoryCatalog`. Category é registrada na criação e não existe comando de reclassificação no Incremento 3A. `expense.subscriptions` é somente uma classificação: recorrência pertence ao Incremento 3B.
...
O adapter HTTP expõe catálogo, preview, registro e consulta mensal somente quando `JARVIS_FINANCIAL_API_ENABLED=true`. O owner vem de `JARVIS_OWNER_ID` no servidor como contexto single-owner temporário; não é autenticação e nunca é aceito ou retornado pelo contrato. A API atribui `origin=IOS` e `financialTimezone=America/Sao_Paulo`. O decoder rejeita propriedades desconhecidas e JSON adicional, o body é limitado, Money usa minor units inteiras e respostas financeiras desabilitam cache. `GET /v1/categories` lista o catálogo de sistema em ordem determinística; `categoryId` é opcional nos fluxos financeiros e o cliente não envia display name ou transaction type da Category.
...
O fingerprint inclui tipo, descrição normalizada, Money, instante já canonicalizado em UTC/microssegundos, origin e timezone atribuídos pelo servidor; somente Expense inclui forma de pagamento. Category presente acrescenta somente seu `CategoryID`; Category ausente preserva byte a byte o algoritmo legado. Duas entradas que diferem apenas na precisão descartada são semanticamente equivalentes; instantes canônicos distintos geram fingerprints distintos. Exclui ID gerado, Clock, display name e timestamps de infraestrutura. A implementação atual ainda gera ID e consulta Clock antes de o store identificar um replay de Income; isso não altera o recurso persistido nem a garantia idempotente. `GET` filtra owner, timezone e intervalo mensal inclusivo/exclusivo, ordenando por `occurred_at DESC, id DESC`; o índice `(user_id, occurred_at DESC, id DESC)` atende à query mista sem índice especulativo por tipo. Filtros de tipo/Category são aplicados somente no iOS nesta entrega.
